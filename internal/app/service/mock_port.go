...
func (m *MockPortRepository) UpdateLocation(ctx context.Context, port *repository.Port) error {
	args := m.Called(port)
	return args.Error(0)
}

func (m *MockPortRepository) GetPort(ctx context.Context, key string) (*repository.Port, error) {
	args := m.Called(key)
	return args.Get(0).(*repository.Port), args.Error(1)
}
